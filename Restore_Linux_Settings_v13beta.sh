#!/bin/bash
#########################################################################################################
# Restore_Linux_Settings V.13beta: Restore Linux Mint settings from >>Backup_Linux_Settings<< script.	#                                          	
#                                                                         								#
...
# >>> For the advanced users: Lines 195-224 in this script could be interesting for fine tuning as well.<<<
##############################################################################################################################################
##############################################################################################################################################
...
[ "$UID" -eq 0 ] || exec sudo bash "$0" "$@" #run script as sudo or su

current_version=$(lsb_release -d | sed -n 's/Description:[\t]//p' | tr " " _)
echo "#########################################################"
echo "This is Version 13 of the Restore Script"
echo "#########################################################"

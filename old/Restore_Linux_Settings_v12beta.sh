#!/bin/bash
#########################################################################################################
# Restore_Linux_Settings V.12beta: Restore Linux Mint settings from >>Backup_Linux_Settings<< script.	#                                          	
#                                                                         								#
...
normal=`tput sgr0`
echo "#########################################################"
echo "This is Version 12 of the Restore Script"
echo "#########################################################"
...
echo ""
restore_methods=("complete (${bold}not${normal} recommended)" "All settings but without home directory (${bold}not${normal} recommended)" "selected setting files and former packages (recommended)" "selected setting files and former packages but without home directory" "reinstall former packages only" "only restore Firefox and Thunderbird settings" "only restore Thunderbird settings" "only restore Firefox settings" "Quit") ## array of restoring options
...
#################################################################################
					"selected setting files and former packages (recommended)") # here we don't restore everything, just the most important things
						...
						;;
#################################################################################
					"selected setting files and former packages but without home directory") # here we don't restore everything, just the most important things
						echo "Will now restore only selected setting files (the most important ones)."
						echo ""
						sleep 3
						mkdir -p $temp_dir

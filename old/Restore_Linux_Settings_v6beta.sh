...
echo ""
restore_methods=("complete (${bold}not${normal} recommended)" "selected setting files and former packages" "reinstall former packages only" "Quit") ## array of restoring options
			...
			do 
				case $method in
					"complete (${bold}not${normal} recommended)") # here we restore everything, this might make problems with incompatible files between distributions
						...
						exit
						;;
					"selected setting files and former packages") # here we don't restore everything, just the most important things

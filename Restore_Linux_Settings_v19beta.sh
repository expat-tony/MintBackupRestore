#!/bin/bash
#########################################################################################################
# Restore_Linux_Settings V.19beta: Restore Linux Mint settings from >>Backup_Linux_Settings<< script.	#                                          	
#                                                                         								#
...
# >>> For the advanced users: Lines 291-318 in this script could be interesting for fine tuning as well.<<<
##############################################################################################################################################
##############################################################################################################################################
...
echo "#########################################################"
echo "This is Version 19beta of the Restore Script"
echo "#########################################################"
...
#################################################################################
					"Complete (${bold}not${normal} recommended)") # here we restore everything, this might make problems with incompatible files between distributions
						...
						if  [ "$help_user" == "n" ]; then
  							echo ""
						elif  [ "$help_user" == "N" ]; then
  							echo ""
						elif [ "$help_user" == "y" ]; then
							echo ""
							...
							echo ""
							echo "================================================================="
							echo "Opening your etc file (please edit accordingly and save the file.)"
...
#################################################################################
					"Only restore Thunderbird settings") # here we don't restore everything, just all settings of Thunderbird (emails and accounts...)
						...
						thunderbird_dirs=$(tar -tvf ""$temp_dir"/"$tar_archive_name"" | grep -Po 'home/.*/.thunderbird/$' | tr -s '\n' ' ') #since --wildcards in tar is not implemented correctly (will not keep owners despite -p option), we have to use this trick.
						...
						sleep 2
						for directory in $thunderbird_dirs ; do # we will use a for loop in case there are multiple users present on the backed up PC.
